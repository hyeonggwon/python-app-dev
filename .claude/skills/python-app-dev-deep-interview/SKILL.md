...
| `interventions.design_per_phase` | 각 phase 의 design 직후 | `false` |
| `interventions.pr_per_phase` | 각 phase 의 pr-create 직후 | `true` |

빠르게 진행 = 모두 `false`, 신중히 = 모두 `true`, 기본 = 위 표 그대로.

> **boolean 정본 = `true`/`false`**. spec.md (YAML) 와 `state.json.interview_spec` (JSON) 양쪽에 같은 표기로 기록한다. orchestrator 의 `_truthy` 가 `on/off`/`yes/no` 도 받아 주지만, 머지 단계에서 문자열로 흘러갈 위험이 있어 정본은 한 가지로 못박는다.

### 2-5. 검증·리뷰 옵션

- `coverage_threshold` (기본 70)
- `mypy_strict` (기본 `false`)
- `security_review` (기본 `false`; `true` 이면 code-review 에서 `security-auditor` 추가 호출)
- `pytest_parallel` (기본 `false`; `true` 이면 `pytest-xdist` 필요)
